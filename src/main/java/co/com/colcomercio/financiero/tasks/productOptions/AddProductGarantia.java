...
public class AddProductGarantia implements Task {
    private static final Logger logger = LogManager.getLogger(AddProductGarantia.class);
    @Override
    public <T extends Actor> void performAs(T actor) {
        ...
        if(TEXT_GARANTIA.isVisibleFor(actor)){
            actor.attemptsTo(
                    ClickOnElement.on(CHECK_SI_GARANTIA)
            );
        }
        actor.attemptsTo(
    ...
    public static AddProductGarantia goToPay() {
        return Tasks.instrumented(AddProductGarantia.class);
    }
}
